...
_DEFAULT_MAX = 2000
_max_entries = max(100, min(int(os.environ.get("GODOT_LOG_MAX_ENTRIES", str(_DEFAULT_MAX))), 50_000))
_lock = Lock()
...
def export_logs(
    *,
    ...
    if format == "csv":
        ...
        return buffer.getvalue(), "text/csv", f"godot-mcp-logs-{stamp}.csv"

    body = json.dumps({"exported_at": datetime.now(UTC).isoformat(), "entries": entries}, indent=2)
    return body, "application/json", f"godot-mcp-logs-{stamp}.json"
