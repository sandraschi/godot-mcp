...
BRIDGE_ACTIONS: list[BridgeAction] = [
    ...
    BridgeAction(
        "import_stl",
        "Import STL mesh into scene as MeshInstance3D",
        ["path", "name", "scale", "position{x,y,z}"],
        ["imported", "name", "vertices", "aabb"],
    ),
    BridgeAction(
        "import_glb",
        "Import GLB/GLTF 3D model (native Godot importer)",
        ["path", "name", "scale", "position{x,y,z}"],
        ["imported", "name", "total_nodes", "mesh_count"],
    ),
    BridgeAction(
        "play_animation",
        "Play or list AnimationPlayer clips on imported GLB",
        ["root_name", "animation", "loop", "speed_scale"],
        ["playing", "animation", "available_animations", "listed", "animations"],
    ),
    BridgeAction(
        "import_obj",
        "Import OBJ mesh with MTL material support",
        ["path", "name", "scale", "position{x,y,z}"],
        ["imported", "name"],
    ),
    BridgeAction(
        "load_velocity_field",
        "Load CFD velocity field CSV into node metadata",
        ["csv_path", "name"],
        ["loaded", "name", "point_count", "bbox"],
    ),
    BridgeAction(
        "spawn_particles",
        "Create GPUParticles3D system",
        ["count", "name", "color", "spread_{x,y,z}"],
        ["spawned", "name", "count"],
    ),
    BridgeAction(
        "animate_streamlines",
        "Animate particles along loaded velocity field",
        ["velocity_field", "particle_system", "speed"],
        ["animated", "particle_system", "speed_multiplier"],
    ),
    BridgeAction(
        "create_camera",
        "Create Camera3D with orbit controls",
        ["name", "position{x,y,z}", "look_at{x,y,z}", "fov"],
        ["created", "name", "fov", "position"],
    ),
    BridgeAction(
        "add_light",
        "Add directional / ambient / omni light source",
        ["type", "name", "intensity", "position{x,y,z}"],
        ["created", "name", "type", "intensity"],
    ),
    BridgeAction(
        "set_material",
        "Assign PBR StandardMaterial3D to mesh node",
        ["node", "color", "roughness"],
        ["set", "node", "color", "roughness"],
    ),
    BridgeAction(
        "export_web", "Export scene to HTML5/WebAssembly", ["output_path"], ["exported", "message", "output_path"]
    ),
    BridgeAction(
        "read_scene_tree",
        "Read full scene hierarchy as nested JSON",
        [],
        ["scene_tree{name,type,path,children[]}", "node_count"],
    ),
    BridgeAction(
        "set_config",
        "Write setting to project.godot ConfigFile",
        ["section", "key", "value"],
    ...
    BridgeAction(
        "add_node",
        "Add a child node to the scene tree",
    ...
    BridgeAction(
        "modify_node",
        "Modify a node property (mass, position, etc.)",
        ["node", "property", "value"],
        ["success"],
        "bonus",
    ),
...
PYTHON_TOOLS: list[PythonTool] = [
    ...
    PythonTool(
        "godot_export_release", "Export Godot project to target platform (web/windows)", "itch", "0.2.1", "MUTATING"
    ),
    ...
    PythonTool("steam_status", "Check steam-mcp connectivity and Steam env", "steam", "0.3.0", "READ_ONLY"),
    PythonTool("steam_checklist", "Steam Direct / release checklist", "steam", "0.3.0", "READ_ONLY"),
    PythonTool("steam_monetization_guide", "Steam pricing and revenue guidance", "steam", "0.3.0", "READ_ONLY"),
    PythonTool("steam_stage_build", "Export Windows build and stage for Steam depot", "steam", "0.3.0", "MUTATING"),
    PythonTool("ship_to_steam_prerelease", "Upload staged build to Steam beta branch", "steam", "0.3.0", "MUTATING"),
    PythonTool("ship_to_steam_release", "Upload staged build to Steam default branch", "steam", "0.3.0", "MUTATING"),
    PythonTool("ship_to_steam", "Full export → stage → Steam upload pipeline", "steam", "0.3.0", "MUTATING"),
    ...
    PythonTool(
        "fleet_worldlabs_get_world", "Get world generation status from worldlabs-mcp", "fleet", "0.2.1", "READ_ONLY"
    ),
    PythonTool("fleet_worldlabs_stage_mesh", "Stage a Marble world mesh in the exchange", "fleet", "0.2.1", "MUTATING"),
    PythonTool(
        "fleet_worldlabs_stage_splat", "Stage a 3D Gaussian splat in the exchange", "fleet", "0.2.1", "MUTATING"
    ),
...
TOOL_CHAINS: list[ToolChain] = [
    ToolChain(
        "spatial-vibe",
        "place_asset",
        "Import and position a 3D asset at a spatial anchor",
        ["godot_import_glb", "godot_set_material"],
    ),
    ToolChain("spatial-vibe", "anchor_light", "Place a dynamic light at a spatial position", ["godot_add_light"]),
    ToolChain(
        "spatial-vibe",
        "wire_trigger",
        "Create a trigger zone between two anchors",
        ["godot_read_scene_tree", "workflow_run"],
    ),
    ToolChain(
        "spatial-vibe",
        "move_node",
        "Reposition an existing node in the scene",
        ["godot_read_scene_tree", "modify_node"],
    ),
    ...
    ToolChain(
        "state-surveiller",
        "reparent",
        "Change a node's parent in the scene tree",
        ["godot_read_scene_tree", "modify_node"],
    ),
    ToolChain("state-surveiller", "set_param", "Modify a property on a scene node", ["modify_node"]),
    ...
    ToolChain(
        ...
        "Design and compose a full game environment from a prompt",
        ["design_game", "generate_game_worlds", "compose_game_scene"],
    ),
    ToolChain(
        "pocket-architect",
        "generate:asset",
        "Search depot and import a 3D asset by description",
        ["artifact_search", "godot_import_glb", "godot_set_material"],
    ),
    ToolChain(
        "pocket-architect",
        "generate:ui_theme",
        "Generate a UI theme via material advisor",
        ["godot_set_config", "prompt_execute"],
    ),
    ToolChain(
        "pocket-architect",
        "generate:behavior",
        "Generate GDScript behavior from spec",
        ["ai_generate_gdscript", "game_eval"],
    ),
...
CHANNELS: list[Channel] = [
    Channel(
        "agent:*",
        "All test agent state changes (stuck, crashed, completed)",
        "~2s",
        '{"agent_id", "status", "scene_state", "last_error"}',
    ),
    Channel(
        "agent:{id}",
        "Specific agent's state changes only",
        "~2s",
        '{"agent_id", "status", "current_action", "duration_sec"}',
    ),
    Channel(
        "logs",
        "Server log entries (ring buffer, 2000 entries)",
        "~0.5s",
        '{"channel": "logs", "data": "timestamp - level - message"}',
    ),
    Channel(
        "frames:{id}",
        "Streamed screenshot frames from a specific agent",
        "~1s",
        '{"channel": "frames:{id}", "data": "base64 PNG"}',
    ),
    Channel(
        "godot:status", "Godot engine periodic status", "~2s", '{"version", "node_count", "fps", "bridge_connected"}'
    ),
    Channel(
        "progress", "Long-running operation progress (generation, export)", "on change", '{"step", "pct", "message"}'
    ),
]
...
ERROR_CODES: list[ErrorCode] = [
    ErrorCode(
        "BRIDGE_DISCONNECTED",
        "Godot engine not running or bridge port 9080 unreachable",
        "Show 'Start Godot' prompt on iOS. Ensure the MCP bridge addon is loaded.",
        503,
    ),
    ErrorCode(
        "ANCHOR_NOT_FOUND",
        "Spatial anchor lost tracking (ARKit tracking state 'limited'/'unavailable')",
        "Ask user to re-scan the physical surface and re-place the anchor.",
        404,
    ),
    ErrorCode(
        "TOOL_FAILED",
        "godot-mcp tool returned an error (invalid params, bridge busy, etc.)",
        "Display the tool error message to the user. Retry with corrected params.",
        400,
    ),
    ErrorCode(
        "INVALID_INTENT",
        "Voice transcription confidence < 0.4 or unknown intent_type for the app",
        "Ask user to rephrase. Show available intent types for their app.",
        400,
    ),
    ErrorCode(
        "AUTH_EXPIRED",
        "JWT token has expired (24h default lifetime)",
        "Re-authenticate via the auth endpoint to get a fresh token.",
        401,
    ),
    ErrorCode(
        "RATE_LIMITED",
        "Too many rapid commands (threshold: 30/min per client)",
        "Back off and retry with exponential backoff.",
        429,
    ),
    ErrorCode(
        "HANDSHAKE_FAILED",
        "Initial WebSocket handshake JSON was invalid or timed out",
        "Send a valid JSON object with 'app' field within 5 seconds of connecting.",
        400,
    ),
    ErrorCode(
        "UPGRADE_REQUIRED",
        "iOS client protocol version is below server minimum",
        "Update the iOS app to the latest version.",
        426,
    ),
]
...
PROTOCOL_SCHEMAS: dict[str, Any] = {
    ...
    "apps": {
        ...
        "pocket-architect": {
            ...
            "payload_schema": {
                "prompt": "str",
                "mode": "environment|asset|ui_theme|behavior|gdscript",
                "constraints": "dict",
                "outputs_requested": "[str]",
            },
...
def get_endpoint_summary() -> str:
    ...
    return "Mobile gateway: ws://0.0.0.0:10993/mobile/v1 | REST: POST /mobile/v1/command | Help: GET /mobile/v1/help"
